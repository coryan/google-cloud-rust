    ...
    F: Future<Output = Result<R, Error>>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let Some(snapshot) = RequestRecorder::current().map(|r| r.t3_snapshot()) else {
...
mod tests {
    ...
    async fn error_with_partial_recorder() -> anyhow::Result<()> {
        ...
        let got = scoped.scope(logging).await;
        assert!(matches!(got, Err(_)), "{got:?}");
        let contents = String::from_utf8(buffer.captured())?;
        let mut s = contents.split('\n');
        let parsed = match (s.next(), s.next()) {
            (Some(line), None) => serde_json::from_str::<Value>(line)?,
