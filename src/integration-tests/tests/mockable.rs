...
mod mocking {
    ...
    use integration_tests::wrapped_inner_client::transport;
    use integration_tests::wrapped_inner_client::client;
    
    use gax::error::Error;
    type Result<T> = std::result::Result<T, Error>;

    async fn application(svc: impl client::FooService) -> Result<Vec<String>> {
    ...
    mockall::mock! {
        ...
        impl client::FooService for ServiceImpl {
    ...
    async fn for_real() -> Result<()> {
        let svc = transport::FooService::builder().build(); 
