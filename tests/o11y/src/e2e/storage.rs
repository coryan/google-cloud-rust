...
pub async fn run() -> anyhow::Result<()> {
    ...
    let required = BTreeSet::from_iter([
        ...
        "delete_bucket",
        "google-cloud-storage::client::Storage::read_object",
        "google-cloud-storage::client::Storage::write_object",
        "google-cloud-storage::client::Storage::open_object",
    ]);
...
async fn send_trace(project_id: &str) -> anyhow::Result<String> {
    ...
    let (provider, _guard) = crate::e2e::set_up_otel_provider(project_id).await?;
